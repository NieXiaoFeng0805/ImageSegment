...
class PraNet(nn.Module):
    ...
    def forward(self, x):
        ...
        ra5_feat = self.agg1(x4_rfb, x3_rfb, x2_rfb)  # 得到经密集并行解码处理后的热图
        # 8倍线性插值上采样
        ...
        # 进入第一个反注意模块
        crop_4 = F.interpolate(ra5_feat, scale_factor=0.25, mode='bilinear')  # 4倍插值下采样得到特征图
        x = -1 * (torch.sigmoid(crop_4)) + 1  # 将得到的特征图经过sigmoid映射为2值图再取反，即反注意机制
        x = x.expand(-1, 2048, -1, -1).mul(x4)  # 在RA中，经反注意机制后的特征图与高分辨率的特征图相乘
        x = self.ra4_conv1(x)  # 经过卷积再次密集特征
        # 改变通道数并激活
        x = F.relu(self.ra4_conv2(x))
        x = F.relu(self.ra4_conv3(x))
        x = F.relu(self.ra4_conv4(x))
        ra4_feat = self.ra4_conv5(x)
        x = ra4_feat + crop_4  # 得到global map下采样图 与 经过RA反注意机制 融合后的特征图
        # 32倍上采样
