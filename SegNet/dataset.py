...
class LabelProcessor:  # 对应data process and load.ipynb 1.处理标签文件中colormap的数据
    ...
    def read_color_map(file_path):  # data process and load.ipynb: 处理标签文件中colormap的数据
        pd_label_color = pd.read_csv(file_path, sep=',')  # 读取每一类别的像素值
        colormap = []
...
class LoadDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(img)
        label = Image.open(label).convert('RGB')  # 标签转为RGB，避免一些不必要的问题
    ...
    def read_file(self, path):
        """从文件夹中读取数据"""
        files_list = os.listdir(path)  # 读取所有文件名称列表
...
label_processor = LabelProcessor(cfg.class_dict_path)  # 实例化

if __name__ == "__main__":
    train = LoadDataset([cfg.TRAIN_ROOT, cfg.TRAIN_LABLE], cfg.crop_size)
